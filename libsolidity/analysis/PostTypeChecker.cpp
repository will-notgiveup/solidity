...
struct CircularContractDependencies: public PostTypeChecker::Checker
{
	...
	void endVisit(ContractDefinition const& _contract) override
	{
		...
		if (contract)
			m_foundCycles.emplace(contract, referencee);
	}
	...
	void finalize() override
	{
		for (auto const& [cycle, referencee]: m_foundCycles)
		{
			...
			ssl.append(string{"Referenced contract"}, cycle->location());
...
private:
	...
	pair<ContractDefinition const*, ASTNode const*> dependenciesAreCyclic(
		ContractDefinition const& _currentContract
	) const
	{
		auto visitor = [&](ContractDefinition const& _contract, util::CycleDetector<ContractDefinition>& _cycleDetector, size_t _depth)
		{
			if (_depth >= 256)
			...
			for (auto& [contract, referencee]: _contract.annotation().contractDependencies)
				if (_cycleDetector.run(*contract))
					return;
		};

		ContractDefinition const* loop = util::CycleDetector<ContractDefinition>(visitor).run(_currentContract);

		if (loop)
			return {loop, _currentContract.annotation().contractDependencies[loop]};
